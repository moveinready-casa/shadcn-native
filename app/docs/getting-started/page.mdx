...
<Steps>
## Use the template or clone the repo
...
Once you are done playing with the demo, you can reset the project by running the following command:
...
Currently Nativewind v5 is in preview and is very buggy. However, if you want to try checkout the [v5 branch](https://github.com/moveinready-casa/expo-starter/tree/v5) on the starter template.
...
<Tabs items={["expo", "pnpm", "npm"]}>
  <Tabs.Tab>
    ```shell copy
    expo install @expo/browser-polyfill tailwind-variants@3.x.x tw-merge@3.x.x twrnc@4.x.x lucide-react-native@0.x.x nativewind@4.x.x react-native-reanimated@~4.x.x react-native-worklets@0.x.x react-native-safe-area-context@5.x.x
